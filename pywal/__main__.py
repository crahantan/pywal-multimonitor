...
def get_args():
    ...
    arg.add_argument(
        ...
        help="Which image or directory to use on Xinerama mode.",
    )
...
def parse_args_exit(parser):
    ...
    if args.i and args.x:
        parser.error("Conflicting arguments -i and -x.")
...
def parse_args(parser):
    ...
    if args.x:
        image_file = image.get(
            args.x, iterative=args.iterative, recursive=args.recursive
        )
        colors_plain = colors.get(image_file, args.l, args.backend, sat=args.saturate)
